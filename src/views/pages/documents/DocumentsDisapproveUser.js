...
const DocumentsDisapproveUser = () => {
    ...
    function getData(value) {
        ...
        axios
            .get(`https://banpho-database.herokuapp.com/documents-status/${id}/${status}`)
            .then((response) => {
    ...
    const handleDelete = () => {
        let id = deleteId;
        axios
            .delete(`https://banpho-database.herokuapp.com/document/${id}`)
            .then((response) => {
                setDeleteId(null);
                setOpenDelete(false);
                getData(user);
    ...
    const handleSaveFormEdit = () => {
        ...
        axios
            .put(`https://banpho-database.herokuapp.com/document/${id}`, {
                title: value[0].name,
    ...
    const uploadFile = async (e) => {
        ...
        try {
            const res = await axios.post('https://banpho-database.herokuapp.com/upload', formData);
            console.log(res);
    ...
    const handleSaveForm = () => {
        ...
        axios
            .post('https://banpho-database.herokuapp.com/document', {
                code: track,
                title: value[0].name,
                detail: value[0].detail,
                file: fileName,
    ...
    function getDataDocument(value) {
        const id = value;
        axios
            .get(`https://banpho-database.herokuapp.com/document-detail/${id}`)
            .then((response) => {
    ...
    function getApprover(value, value1) {
        ...
        axios
            .get(`https://banpho-database.herokuapp.com/documents-get-approver/${code}/${version}`)
            .then((response) => {
                if (response) {
                    let value = response.data.data;
                    setApprover(value);
    ...
    function getDisapprove(value) {
        const code = value;
        axios
            .get(`https://banpho-database.herokuapp.com/documents-get-disapprover/${code}`)
            .then((response) => {
                if (response) {
                    let value = response.data.data;
    ...
    function handleDownload(path) {
        ...
        const download_url = `https://banpho-database.herokuapp.com/download-file?file_path=${file_path}`;
