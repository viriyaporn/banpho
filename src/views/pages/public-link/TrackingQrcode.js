...
const TrackingQrcode = () => {
    ...
    useEffect(() => {
        axios
            .get(`https://banpho-database.herokuapp.com/tracking-data/${params}`)
            .then((response) => {
    ...
    const getEquipment = () => {
        axios
            .get(`https://banpho-database.herokuapp.com/tracking-item/${params}`)
            .then((response) => {
    ...
    return (
        <div>
            <Grid sx={{ textAlign: 'center' }}>
                ...
                <QRCodeSVG
                    value={` https://banpho-webapp.herokuapp.com/tracking-link?track=${params}`}
                    size={350}
                    bgColor={'#ffffff'}
                    fgColor={'#000000'}
                    level={'L'}
